...
async function processMessages(job: Job<ProcessMessagesJob>): Promise<void> {
  ...
  if (!tenant || tenant.status !== 'active') {
          logger.warn({ tenantId }, 'Tenant not found or not active')
          return
  }
  ...
  const combinedText = processedTexts.join('\n')
      if (!combinedText && !imageUrl) {
              logger.warn({ conversationId }, 'No text or image to process')
              return
      }
  ...
  let replyText: string
      try {
              ...
              logger.error({ err, tenantId, conversationId }, 'generateReply failed - check AI API keys')
              return
      }

  if (!replyText || replyText.trim() === '') {
          logger.warn({ conversationId }, 'generateReply returned empty, skipping')
          return
  }

  try {
          await sendMessage(channel, senderId, replyText)
          logger.info({ conversationId, senderId }, 'Message sent successfully')
  } catch (err) {
